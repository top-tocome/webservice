...
//添加拖拽事件
var dz = document.getElementById('modal-box-content');
...
function blink()
{
    document.getElementById('modal-box-content').style.borderColor = 'gray';
}

//ajax上传文件
function upload(){
    if(document.getElementsByTagName('tbody')[1].hasChildNodes()==false){
        document.getElementById('modal-box-content').style.borderColor = 'red';
        setTimeout(blink,200);
        return false;
    }
    var data=Dragfiles(); //获取formData
    fileUpload(localStorage.getItem("path"),data)
}
